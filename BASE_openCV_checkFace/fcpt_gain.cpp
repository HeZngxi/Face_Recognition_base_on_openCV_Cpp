#include "fcpt_gain.h"
#include "ui_fcpt_gain.h"

fcpt_Gain::fcpt_Gain(const QString name,const QString gender,const QString age,const QString id,const QString phone,QWidget* parent) :
    QDialog(parent),
    ui(new Ui::fcpt_Gain),
    mname(name),
    mgender(gender),
...
void fcpt_Gain::on_pushButton_clicked()
{
    if(isCameraOpen==false){
       ...
       connect(frameTimer, &QTimer::timeout, this, &fcpt_Gain::updateFrame);
    ...
    if (capture.read(frame)) {
      ...
      if (!faces.empty() && !hasCapturedFace){
        for (const auto& face : faces) {
            ...
            // 准备 SQL 插入语句
//            QSqlQuery query;
//            query.prepare("INSERT INTO faces (id, name,gender,age,phone,face_features) VALUES (:id,:name, :gender,:age,:phone, :face_features)");

//            // 绑定参数
//            query.bindValue("id",mid);
//            query.bindValue(":name", mname);
//            query.bindValue(":gender", mgender);
//            query.bindValue("age",mage);
//            query.bindValue("phone",mphone);
//            query.bindValue(":face_features", face_feature);
...
void fcpt_Gain::drawFaceRectangles(cv::Mat &grayFrame) {
